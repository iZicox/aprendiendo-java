...
public class Main {
	...
	public static void main(String[] args) {
		...
		System.out.println("**********************\n afinar\n***************************");
		afinarInstrumentos(lista.toArray(new Instrumento[0]));
		
		Instrumento[] listaArray = lista.toArray(new Instrumento[0]);
		
		
	}
	
	private static void afinarInstrumentos(Instrumento lista[]) {
		for(Instrumento instrumento: lista) {
			if(instrumento instanceof Cuerda) {
				((Cuerda) instrumento).afinar();
			}
			
			if(instrumento instanceof Timbales) {
				((Timbales) instrumento).afinar();
			}
