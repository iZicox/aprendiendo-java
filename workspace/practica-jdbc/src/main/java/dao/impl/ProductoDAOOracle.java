...
public class ProductoDAOOracle implements IProductoDAO {
    ...
    public Producto buscar(Connection con, long id) throws SQLException{
        ...
        try(PreparedStatement pstmt = con.prepareStatement(query)){
            ...
            try(ResultSet rs = pstmt.executeQuery()){
                if (rs.next()){
                    return new Producto(rs.getLong("id"), rs.getString("barcode"), rs.getString("nombre"), rs.getDouble("precio"));
                }
            }
        }
        return null;
    ...
    public List<Producto> listar(Connection con) throws SQLException{
        ...
        // Creamos un ArrayList de objetos Producto con todos los productos de la base de datos
        try(PreparedStatement pstmt = con.prepareStatement(query)){
            try(ResultSet rs = pstmt.executeQuery()){
                while (rs.next()){
